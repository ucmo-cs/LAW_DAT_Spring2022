...
/*
    ...
    It's parent component in this project is 'App.js'. It's child components are 'Box.js'.
*/
...
//Contructor for the page. This is wehere props are set and values used are dcelcared
class Section extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
        total: 0, //total for the current section
        active: false //state for if accordian is open or closed
    }
    
    //sets handler to current instance
    //handeler is then called to get the current section total and display it
    this.handler = this.handler.bind(this)
}

    // This function receives new input values from it's child component and passes those values to its parent component (App)
    ...
    render() {
      ...
      const accordianContent = this.props.contents.map(({ lineNumber, description, maxVal }) => (
        ...
        <Box key={lineNumber} id={lineNumber} Title={description} options={actions} handler={this.handler} maxVal = {maxVal}/>
  ))
